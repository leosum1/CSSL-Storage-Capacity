...
for i in `ls | grep 54`; do
	...
	rm -rf date1.out
	sed -n -e '/BEGIN Iteration/,/END Iteration/p' $i | sed -n -e '/POSTSTATS =-=-=-=-=-= df -x -m/,/--- EXE-TIME of /p' | sed -n -e '/vs1_/p' | sed -n -e 's/[A-Z][A-Z]//gp' | sed -n -e 's/%//gp' | sed -n -e 's/\/vol\///gp' | sed -n -e 's/\///gp' | awk '{print $1","$2","$3","$4}' > vol.out
